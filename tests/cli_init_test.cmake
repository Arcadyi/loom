...
if(NOT result EQUAL 0)
    message(FATAL_ERROR "respin init failed:\n${output}\n${error}")
endif()

file(READ "${TEST_DIR}/CMakeLists.txt" cmake_contents)
if(NOT cmake_contents MATCHES "# respin: begin")
    message(FATAL_ERROR "respin init did not add its marked CMake block")
endif()
file(READ "${TEST_DIR}/respin.json" manifest)
if(NOT manifest MATCHES "com[.]example[.]Existing")
    message(FATAL_ERROR "respin init did not infer the QML URI")
endif()
...
if(NOT second_result EQUAL 0)
    message(FATAL_ERROR "A second respin init should be idempotent")
endif()

file(READ "${TEST_DIR}/CMakeLists.txt" second_cmake_contents)
string(REGEX MATCHALL "# respin: begin" markers "${second_cmake_contents}")
list(LENGTH markers marker_count)
if(NOT marker_count EQUAL 1)
    message(FATAL_ERROR "respin init duplicated its CMake integration")
endif()

# An existing respin.json is the source of truth. Re-inferring from the
# CMakeLists regex could emit a block naming a different target than the one
# `respin dev` reads out of the manifest, leaving the two permanently at odds.
...
file(WRITE "${DIVERGED_DIR}/respin.json" [=[
{
...
if(NOT diverged_cmake MATCHES "TARGET manifestTarget")
    message(FATAL_ERROR
        "respin init ignored the existing manifest and re-inferred from CMakeLists:\n"
        "${diverged_cmake}")
endif()
if(diverged_cmake MATCHES "TARGET inferredTarget")
    message(FATAL_ERROR "respin init wrote a target that contradicts respin.json")
endif()
