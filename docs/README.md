...
| [styling/components.md](styling/components.md) | `Loom.Controls`: layout (`Box`, `Row`, `Col`, `Grid`, `Scroll`), form (`Button`, `Field`, `CheckBox`, `Switch`, `RadioButton`, `Slider`, `Select`), display (`Icon`, `Label`, `Divider`, `Spacer`, `ListRow`) — and the part-style convention for reaching a control's sub-delegates |
